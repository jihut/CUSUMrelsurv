...
// Functions to simulate excess times in situations where the hazard switches to the out-of-control at a given time point for the additive alternative, assuming non-negative excess hazard.
...
double out_of_control_add_excess_non_neg_cumulative_excess_hazard_piecewise_func_single(
  ...
  vec new_haz_vec = baseline_vec * exp(linear_predictor) + gamma; // relevant hazard values during these intervals
  uvec negative_indices = arma::find(new_haz_vec < 0);
  new_haz_vec.elem(negative_indices).fill(0); // assuming non-negative excess hazard
  vec pre_cumulative_haz_vec = cumsum(new_haz_vec % interval_length_partition_t_vec); // compute cumulative hazard at these time points
  vec front_cumulative_haz_vec = {0};
  vec cumulative_haz_vec = join_cols(front_cumulative_haz_vec, pre_cumulative_haz_vec);
  int index = find_interval_single(t, partition_t_vec); // find in which time partition the evaluated time point is located
  double relevant_cumulative_excess_hazard_piecewise = cumulative_haz_vec(index) + (t - partition_t_vec(index)) * new_haz_vec(index); // since the hazard itself is piecewise constant
  
  return relevant_cumulative_excess_hazard_piecewise;
}

double final_add_excess_non_neg_cumulative_excess_hazard_piecewise_func_single_in_and_out(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
    double t,
    double arrival_time,
  ...
  if (eta >= arrival_time) { // check if the observation arrives before the change or not
    if (t <= (eta - arrival_time)) { // the cumulative hazard for the given observation at the evaluated time point is before the jump to out-of-control
      ...
      result = (
        ...
        ) * exp(linear_predictor) + out_of_control_add_excess_non_neg_cumulative_excess_hazard_piecewise_func_single(
            partition_t_vec,
            baseline_vec,
            t,
            linear_predictor,
            gamma
        ) - out_of_control_add_excess_non_neg_cumulative_excess_hazard_piecewise_func_single(
            partition_t_vec,
            baseline_vec,
            time_duration_in_control,
            linear_predictor,
    ...
    result = out_of_control_add_excess_non_neg_cumulative_excess_hazard_piecewise_func_single(
      partition_t_vec,
      baseline_vec,
      t,
      linear_predictor,
...
double excess_function_to_solve_add_excess_non_negative_piecewise_in_and_out(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
    double t,
    double u,
  ...
  double func_eval_at_t = final_add_excess_non_neg_cumulative_excess_hazard_piecewise_func_single_in_and_out(
    partition_t_vec,
    baseline_vec,
    t,
    arrival_time,
  ...
  double final_func_to_solve = 1 - exp(-func_eval_at_t) - u; // inverse transform
  
  return final_func_to_solve;
  
}

double excess_brent_add_excess_non_neg_piecewise_in_and_out(double x1, double x2, double u, double tol, double arrival_time, double linear_predictor, double gamma, double eta, arma::vec& partition_t_vec, arma::vec& baseline_vec) { // Implementation of Brent's method from the book "Numerical Recipes: The Art of Scientific Computing"
  ...
  double a = x1, b = x2, c = x2, d, e, fa = excess_function_to_solve_add_excess_non_negative_piecewise_in_and_out(partition_t_vec, baseline_vec, a, u, arrival_time, linear_predictor, gamma, eta), fb = excess_function_to_solve_add_excess_non_negative_piecewise_in_and_out(partition_t_vec, baseline_vec, b, u, arrival_time, linear_predictor, gamma, eta), fc, p, q, r, s, tol1, xm;
  ...
  for (int iter = 0; iter < ITMAX; iter++) {
    ...
    fb = excess_function_to_solve_add_excess_non_negative_piecewise_in_and_out(partition_t_vec, baseline_vec, b, u, arrival_time, linear_predictor, gamma, eta);
...
double single_excess_sim_add_excess_non_neg_piecewise_in_and_out(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
  ...
  double func_value_at_end = excess_function_to_solve_add_excess_non_negative_piecewise_in_and_out(
    partition_t_vec, 
    baseline_vec,
    max_follow_up,
    u,
  ...
  if (func_value_at_end < 0) {
    ...
    time_sim = excess_brent_add_excess_non_neg_piecewise_in_and_out(
      0.0,
      max_follow_up,
      u,
      1e-7,
...
//' @export
// [[Rcpp::export]]
arma::vec vec_excess_sim_add_excess_non_neg_piecewise_in_and_out(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
  ...
  for (int i = 0; i < n_iterations; i++) {
    ...
    time_excess_sim(i) = single_excess_sim_add_excess_non_neg_piecewise_in_and_out(
      partition_t_vec,
      baseline_vec,
      max_follow_up_vec(i),
      u_vec(i),
