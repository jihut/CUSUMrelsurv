...
// Function to calculate the CUSUM chart with the additive alternative exact.
...
//' @export
// [[Rcpp::export]]
NumericMatrix cusum_add_exact_r_t_piecewise(
  ...
  for (int i = 0; i < length_t_grid; i++) {
    ...
    if (length_of_index == 0) { // if no one has arrived
      ...
      if (index_pop_old.size() != 0) {
        ...
        arma::vec h0_vec = pop_hazard_old + excess_hazard_old;
        arma::vec h1_vec = h0_vec + gamma;
        
        arma::vec log_haz_ratio_vec = log(h1_vec / h0_vec); 
