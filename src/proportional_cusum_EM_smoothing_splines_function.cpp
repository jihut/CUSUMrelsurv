...
// Functions to calculate the CUSUM chart with the proportional alternative when the estimated in-control comes from the semi-parametric model by Perme, Henderson and Stare (2009)
// implemented in relsurv package.
// Corresponds to method = "EM" when using the function rsadd(...).

arma::vec x_i_vec_func(arma::vec& time_obs_vec, arma::vec& arrival_time_vec, double& t) {
  
  // Function to calculate the current time at risk at a calendar time t. 
  // Will become negative for individuals not arrived yet, but these will not be a part of CUSUM calculations.
  
  int n_iterations = time_obs_vec.size();
  
  arma::vec x_i_vec(n_iterations);
  
  for (int i = 0; i < n_iterations; i++) {
    
    x_i_vec(i) = std::min(time_obs_vec(i), t - arrival_time_vec(i));
...
arma::vec delta_i_vec_func(arma::vec& time_obs_vec, arma::vec& arrival_vec, arma::vec& delta_i_vec, double& t) {
  
  // Function to calculate the event indicator at a calendar time t
  
  int n_iterations = time_obs_vec.size();
  
  arma::vec delta_i_vec_t(n_iterations);
  
  arma::vec x_i_vec = x_i_vec_func(time_obs_vec, arrival_vec, t);
  
  for (int i = 0; i < n_iterations; i++) {
    
    int delta_i = 0;
    
    bool first_cond = time_obs_vec(i) <= x_i_vec(i);
    bool second_cond = delta_i_vec(i) == 1; // the observation has to experience the event at the end in order for this to have a chance of being 1.
    ...
    delta_i_vec_t(i) = delta_i;
    
  }
  
  return delta_i_vec_t;
...
//' @export
// [[Rcpp::export]]
NumericMatrix cusum_prop_r_t_EM_smoothing_spline(
    Rcpp::Function cumulative_baseline_haz_vec_general_func,
    Rcpp::Function baseline_haz_vec_general_func,
    double start_year,
    arma::vec age_vec,
    arma::vec gender_vec,
    arma::mat x_matrix,
  ...
  for (int i = 0; i < length_t_grid; i++) {
    ...
    if (length_of_index == 0) { // if no one has arrived
      ...
      if (length_of_index_pop != 0) {
        ...
        NumericVector baseline_haz_vec_rcpp = baseline_haz_vec_general_func(new_time_obs_vec_pop); // calculate the estimated baseline hazard for these observations at the observed times
        arma::vec baseline_haz_vec = as<arma::vec>(baseline_haz_vec_rcpp);
      ...
      if (index_pop_old.size() != 0) {
        ...
        NumericVector cumulative_baseline_haz_vec_rcpp = cumulative_baseline_haz_vec_general_func(at_risk_time_vec);
        
        arma::vec cumulative_baseline_haz_vec = as<vec>(cumulative_baseline_haz_vec_rcpp);
        ...
        arma::vec cumulative_baseline_haz_vec = as<arma::vec>(cumulative_baseline_haz_vec_rcpp);
