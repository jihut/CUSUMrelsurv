...
double excess_function_to_solve_acc_time_lin_piecewise_out_traditional(
  ...
  double func_eval_at_t = out_of_control_acc_time_lin_cumulative_excess_hazard_add_piecewise_func_single_traditional(
    partition_t_vec,
    baseline_vec,
    t,
    k
...
double excess_brent_acc_time_lin_piecewise_out_traditional(double x1, double x2, double u, double tol, double linear_predictor, double k, arma::vec& partition_t_vec, arma::vec& baseline_vec) { // Implementation of Brent's method from the book "Numerical Recipes: The Art of Scientific Computing"
  ...
  double a = x1, b = x2, c = x2, d, e, fa = excess_function_to_solve_acc_time_lin_piecewise_out_traditional(partition_t_vec, baseline_vec, a, u, linear_predictor, k), fb = excess_function_to_solve_acc_time_lin_piecewise_out_traditional(partition_t_vec, baseline_vec, b, u, linear_predictor, k), fc, p, q, r, s, tol1, xm;
  ...
  for (int iter = 0; iter < ITMAX; iter++) {
    ...
    fb = excess_function_to_solve_acc_time_lin_piecewise_out_traditional(partition_t_vec, baseline_vec, b, u, linear_predictor, k);
...
double single_excess_sim_acc_time_lin_piecewise_out_traditional(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
  ...
  double func_value_at_end = excess_function_to_solve_acc_time_lin_piecewise_out_traditional(
    partition_t_vec, 
    baseline_vec,
    max_follow_up,
    u,
  ...
  if (func_value_at_end < 0) {
    ...
    time_sim = excess_brent_acc_time_lin_piecewise_out_traditional(
      0.0,
      max_follow_up,
      u,
      1e-7,
...
//' @export
// [[Rcpp::export]]
arma::vec vec_excess_sim_acc_time_lin_piecewise_out_traditional(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
  ...
  for (int i = 0; i < n_iterations; i++) {
    ...
    time_excess_sim(i) = single_excess_sim_acc_time_lin_piecewise_out_traditional(
      partition_t_vec,
      baseline_vec,
      max_follow_up_vec(i),
      u_vec(i),
