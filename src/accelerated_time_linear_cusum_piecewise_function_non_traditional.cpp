...
// Functions to calculate the CUSUM chart with the linear accelerated time alternative.

arma::vec out_of_control_acc_time_lin_baseline_excess_hazard_piecewise_func_non_traditional(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec, // exp(chi)
    arma::vec& t_vec,
    double k
) {
  
  arma::vec new_t_vec = k * t_vec; // linear accelerated time on the baseline
  
  arma::vec relevant_baseline_excess_hazard_piecewise = baseline_excess_hazard_piecewise_func(partition_t_vec, baseline_vec, new_t_vec);
  
  return relevant_baseline_excess_hazard_piecewise;
  
}

arma::vec out_of_control_acc_time_lin_cumulative_baseline_excess_hazard_piecewise_func_non_traditional(
    arma::vec& partition_t_vec, 
  ...
  arma::vec new_t_vec = k * t_vec; // similar as above
  
  arma::vec relevant_cumulative_baseline_excess_hazard_piecewise = cumulative_baseline_excess_hazard_piecewise_func(partition_t_vec, baseline_vec, new_t_vec) / k; // since the time argument in the hazard function is accelerated
...
//' @export
// [[Rcpp::export]]
NumericMatrix cusum_acc_time_lin_r_t_piecewise_non_traditional(
    arma::vec partition_t_vec,
    arma::vec baseline_vec,
    double start_year,
    arma::vec age_vec,
  ...
  arma::vec out_of_control_excess_hazard_old; 
  ...
  for (int i = 0; i < length_t_grid; i++) {
    ...
    if (length_of_index == 0) { // if no one has arrived
      ...
      if (length_of_index_pop != 0) {
        ...
        new_age_floor_vec_pop.elem(index_older_than_110).fill(110); // since the population table is modified so that 110+ corresponds to one single value 110
        ...
        new_year_floor_vec_pop.elem(index_year_later_than_end_year_table).fill(end_year_table); // in case the calendar year of an observation is larger than the available year in population table
        ...
        arma::vec current_linear_predictor_vec = exp(new_x_matrix_pop * beta_vec); 
        ...
        arma::vec excess_hazard_vec = baseline_haz_vec % current_linear_predictor_vec;
        excess_hazard_old = join_cols(excess_hazard_old, excess_hazard_vec);
        
        arma::vec out_of_control_baseline_haz_vec = out_of_control_acc_time_lin_baseline_excess_hazard_piecewise_func_non_traditional(partition_t_vec, baseline_vec, new_time_obs_vec_pop, k); // calculate the estimated baseline hazard for these observations at the observed times
        arma::vec out_of_control_excess_hazard_vec = out_of_control_baseline_haz_vec % current_linear_predictor_vec;
        out_of_control_excess_hazard_old = join_cols(out_of_control_excess_hazard_old, out_of_control_excess_hazard_vec);
      ...
      if (index_pop_old.size() != 0) {
        ...
        arma::vec h1_vec = pop_hazard_old + out_of_control_excess_hazard_old; // out-of-control hazard for observations experiencing event at the end of follow up
        ...
        arma::vec new_linear_predictor_vec = exp(new_x_matrix * beta_vec);
        
        arma::vec H0_vec = cumulative_baseline_excess_hazard_piecewise_func(partition_t_vec, baseline_vec, at_risk_time_vec) % new_linear_predictor_vec;
        arma::vec H1_vec = out_of_control_acc_time_lin_cumulative_baseline_excess_hazard_piecewise_func_non_traditional(partition_t_vec, baseline_vec, at_risk_time_vec, k) % new_linear_predictor_vec;
