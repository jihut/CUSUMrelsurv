...
// Function to calculate the CUSUM chart with the additive alternative (using first order Taylor expansion).
  ...
  for (int i = 0; i < length_t_grid; i++) {
    ...
    if (length_of_index == 0) { // if no one has arrived
      ...
      if (index_pop_old.size() != 0) {
        ...
        arma::vec approx_log_haz_ratio_vec = gamma / (pop_hazard_old + excess_hazard_old); // first term of Taylor expansion of log(h_1 / h_0) where h_1 = h_p + h_e + gamma while h_0 = h_p + h_e. 
        ...
        double r_t = sum(event_vec % approx_log_haz_ratio_vec) - sum(diff_cumulative_haz_vec);
