...
// Function to calculate the CUSUM chart with the additive alternative, assuming that the total hazard (population + excess) is non-negative.
...
//' @export
// [[Rcpp::export]]
NumericMatrix cusum_add_total_haz_non_neg_exact_r_t_piecewise(
  ...
  for (int i = 0; i < length_t_grid; i++) {
    ...
    if (length_of_index == 0) { // if no one has arrived
      ...
      if (index_pop_old.size() != 0) {
        ...
        arma::vec h1_vec = pop_hazard_old + excess_hazard_old + gamma; // out-of-control hazard for observations experiencing event at the end of follow up
        arma::uvec indices_negative_h1_vec = arma::find(h1_vec < 0);
        ...
        log_haz_ratio_vec.elem(indices_negative_h1_vec).fill(0); // those with negative total hazard (although the observation can in theory experience an event in simulation setting since population time and excess time are simulated independently) is assumed to not experience an event at the given time point.
        ...
        arma::vec diff_cumulative_haz_vec = gamma * at_risk_time_vec;
