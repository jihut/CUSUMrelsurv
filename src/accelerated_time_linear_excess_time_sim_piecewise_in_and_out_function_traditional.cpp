...
double out_of_control_acc_time_lin_cumulative_excess_hazard_add_piecewise_func_single_traditional(
  ...
  double relevant_cumulative_baseline_excess_hazard_piecewise = cumulative_baseline_excess_hazard_piecewise_func_single(partition_t_vec, baseline_vec, new_t);
  
  return relevant_cumulative_baseline_excess_hazard_piecewise;
}

double final_acc_time_lin_cumulative_baseline_excess_hazard_piecewise_func_single_in_and_out_traditional(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
    double t,
    double arrival_time,
  ...
  if (eta >= arrival_time) { // check if the observation arrives before the change or not
    if (t <= (eta - arrival_time)) { // the cumulative hazard for the given observation at the evaluated time point is before the jump to out-of-control
      ...
      result = (
        ...
        ) + out_of_control_acc_time_lin_cumulative_excess_hazard_add_piecewise_func_single_traditional(
            partition_t_vec,
            baseline_vec,
            t,
            k
    ...
    result = out_of_control_acc_time_lin_cumulative_excess_hazard_add_piecewise_func_single_traditional(
      partition_t_vec,
      baseline_vec,
      t,
      k
...
double excess_function_to_solve_acc_time_lin_piecewise_in_and_out_traditional(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
    double t,
    double u,
  ...
  double func_eval_at_t = final_acc_time_lin_cumulative_baseline_excess_hazard_piecewise_func_single_in_and_out_traditional(
    partition_t_vec,
    baseline_vec,
    t,
    arrival_time,
...
double excess_brent_acc_time_lin_piecewise_in_and_out_traditional(double x1, double x2, double u, double tol, double arrival_time, double linear_predictor, double k, double eta, arma::vec& partition_t_vec, arma::vec& baseline_vec) { // Implementation of Brent's method from the book "Numerical Recipes: The Art of Scientific Computing"
  ...
  double a = x1, b = x2, c = x2, d, e, fa = excess_function_to_solve_acc_time_lin_piecewise_in_and_out_traditional(partition_t_vec, baseline_vec, a, u, arrival_time, linear_predictor, k, eta), fb = excess_function_to_solve_acc_time_lin_piecewise_in_and_out_traditional(partition_t_vec, baseline_vec, b, u, arrival_time, linear_predictor, k, eta), fc, p, q, r, s, tol1, xm;
  ...
  for (int iter = 0; iter < ITMAX; iter++) {
    ...
    fb = excess_function_to_solve_acc_time_lin_piecewise_in_and_out_traditional(partition_t_vec, baseline_vec, b, u, arrival_time, linear_predictor, k, eta);
...
double single_excess_sim_acc_time_lin_piecewise_in_and_out_traditional(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
  ...
  double func_value_at_end = excess_function_to_solve_acc_time_lin_piecewise_in_and_out_traditional(
    partition_t_vec, 
    baseline_vec,
    max_follow_up,
    u,
  ...
  if (func_value_at_end < 0) {
    ...
    time_sim = excess_brent_acc_time_lin_piecewise_in_and_out_traditional(
      0.0,
      max_follow_up,
      u,
      1e-7,
...
//' @export
// [[Rcpp::export]]
arma::vec vec_excess_sim_acc_time_lin_piecewise_in_and_out_traditional(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
  ...
  for (int i = 0; i < n_iterations; i++) {
    ...
    time_excess_sim(i) = single_excess_sim_acc_time_lin_piecewise_in_and_out_traditional(
      partition_t_vec,
      baseline_vec,
      max_follow_up_vec(i),
      u_vec(i),
