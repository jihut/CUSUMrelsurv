...
arma::vec out_of_control_acc_time_lin_baseline_excess_hazard_piecewise_func_traditional(
  ...
  arma::vec new_t_vec = k * t_vec; // linear accelerated time on the cumulative baseline
  
  arma::vec relevant_baseline_excess_hazard_piecewise = k * baseline_excess_hazard_piecewise_func(partition_t_vec, baseline_vec, new_t_vec);
  
  return relevant_baseline_excess_hazard_piecewise;
  
}

arma::vec out_of_control_acc_time_lin_cumulative_baseline_excess_hazard_piecewise_func_traditional(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
    arma::vec& t_vec,
    double k
  ...
  arma::vec new_t_vec = k * t_vec;
  
  arma::vec relevant_cumulative_baseline_excess_hazard_piecewise = cumulative_baseline_excess_hazard_piecewise_func(partition_t_vec, baseline_vec, new_t_vec); // since the time argument in the cumulative hazard function is accelerated
...
//' @export
// [[Rcpp::export]]
NumericMatrix cusum_acc_time_lin_r_t_piecewise_traditional(
    arma::vec partition_t_vec,
    arma::vec baseline_vec,
    double start_year,
    arma::vec age_vec,
  ...
  for (int i = 0; i < length_t_grid; i++) {
    ...
    if (length_of_index == 0) { // if no one has arrived
      ...
      if (length_of_index_pop != 0) {
        ...
        arma::vec out_of_control_baseline_haz_vec = out_of_control_acc_time_lin_baseline_excess_hazard_piecewise_func_traditional(partition_t_vec, baseline_vec, new_time_obs_vec_pop, k); // calculate the estimated baseline hazard for these observations at the observed times
      ...
      if (index_pop_old.size() != 0) {
        ...
        arma::vec H1_vec = out_of_control_acc_time_lin_cumulative_baseline_excess_hazard_piecewise_func_traditional(partition_t_vec, baseline_vec, at_risk_time_vec, k) % new_linear_predictor_vec;
