...
// Functions to simulate excess times in situations where the hazard starts out at the out-of-control at a given time point for the proportional alternative.

double excess_function_to_solve_prop_piecewise_out(
  ...
  double func_eval_at_t = out_of_control_prop_cumulative_baseline_excess_hazard_piecewise_func_single(
    partition_t_vec,
    baseline_vec,
    t,
    rho
...
double excess_brent_prop_piecewise_out(double x1, double x2, double u, double tol, double linear_predictor, double rho, arma::vec& partition_t_vec, arma::vec& baseline_vec) { // Implementation of Brent's method from the book "Numerical Recipes: The Art of Scientific Computing"
  ...
  double a = x1, b = x2, c = x2, d, e, fa = excess_function_to_solve_prop_piecewise_out(partition_t_vec, baseline_vec, a, u, linear_predictor, rho), fb = excess_function_to_solve_prop_piecewise_out(partition_t_vec, baseline_vec, b, u, linear_predictor, rho), fc, p, q, r, s, tol1, xm;
  ...
  for (int iter = 0; iter < ITMAX; iter++) {
    ...
    fb = excess_function_to_solve_prop_piecewise_out(partition_t_vec, baseline_vec, b, u, linear_predictor, rho);
...
double single_excess_sim_prop_piecewise_out(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
  ...
  double func_value_at_end = excess_function_to_solve_prop_piecewise_out(
    partition_t_vec, 
    baseline_vec,
    max_follow_up,
    u,
  ...
  if (func_value_at_end < 0) {
    ...
    time_sim = excess_brent_prop_piecewise_out(
      0.0,
      max_follow_up,
      u,
      1e-7,
...
//' @export 
// [[Rcpp::export]]
arma::vec vec_excess_sim_prop_piecewise_out(
    arma::vec& partition_t_vec, 
    arma::vec& baseline_vec,
  ...
  for (int i = 0; i < n_iterations; i++) {
    ...
    time_excess_sim(i) = single_excess_sim_prop_piecewise_out(
      partition_t_vec,
      baseline_vec,
      max_follow_up_vec(i),
      u_vec(i),
