...
#' 
#' Function to simulate threshold for CUSUM with proportional alternative under piecewise constant baseline hazard (with covariates simulating via boostrapping).
...
#' @export
sim_bootstrap_cusum_prop_threshold_piecewise_function_parallel <- function(
    partition_t_vec,
    baseline_vec,
    n_iterations,
    n_cores = NULL,
  ...
  store_matrix <- foreach::foreach(
    ...
    for (j in 1:n_elements_rho_vec) {
      cusum.chart <- cusum_prop_r_t_piecewise(
        partition_t_vec = partition_t_vec,
        baseline_vec = baseline_vec,
