...
#' @export
sim_bootstrap_cusum_acc_time_lin_non_traditional_threshold_piecewise_function_parallel <- function(
    partition_t_vec,
    baseline_vec,
    n_iterations,
    n_cores = NULL,
  ...
  store_matrix <- foreach::foreach(
    ...
    for (j in 1:n_elements_k_vec) {
      cusum.chart <- cusum_acc_time_lin_r_t_piecewise_non_traditional(
        partition_t_vec = partition_t_vec,
        baseline_vec = baseline_vec,
