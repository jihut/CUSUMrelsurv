...
# Function to simulate the max values of the CUSUM chart under in-control to estimate the threshold based on this result with the additive alternative assuming non-negative excess hazard.
# Here, datasets used to calculate the CUSUM charts are simulated using the true covariate distribution. 

#' Function to simulate threshold for CUSUM with additive alternative assuming non-negative excess hazard under piecewise constant baseline hazard. 
#' 
#' Function to simulate threshold for CUSUM with additive alternative assuming non-negative excess under piecewise constant baseline hazard (with covariates simulating via prespecified instructions).
...
#' @param gamma_vec A vector of values of the additive constant for the out-of-control hazard model.  
...
#' @export
sim_predefined_obs_func_cusum_add_excess_non_neg_threshold_piecewise_function_parallel <- function(
    sim_obs_func,
    n_iterations,
    n_cores = NULL,
    gamma_vec,
  ...
  n_elements_gamma_vec <- length(gamma_vec)
  ...
  store_matrix <- foreach::foreach(
    ...
    store_vec <- numeric(n_elements_gamma_vec)
    ...
    for (j in 1:n_elements_gamma_vec) {
      cusum_chart <- cusum_add_excess_non_neg_r_t_piecewise(
        partition_t_vec = sim_obs_run$partition_t_vec,
        baseline_vec = sim_obs_run$baseline_vec,
        start_year = sim_obs_run$start_year,
        age_vec = sim_obs_run$age_vec,
  ...
  colnames(store_matrix) <- paste("gamma_", gamma_vec, sep = "")
