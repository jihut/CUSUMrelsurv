...
#' @export
sim_predefined_obs_func_cusum_acc_time_lin_threshold_piecewise_function_parallel_non_traditional <- function(
    sim_obs_func,
    n_iterations,
    n_cores = NULL,
    k_vec,
  ...
  store_matrix <- foreach::foreach(
    ...
    for (j in 1:n_elements_k_vec) {
      cusum_chart <- cusum_acc_time_lin_r_t_piecewise_non_traditional(
        partition_t_vec = sim_obs_run$partition_t_vec,
        baseline_vec = sim_obs_run$baseline_vec,
        start_year = sim_obs_run$start_year,
        age_vec = sim_obs_run$age_vec,
