...
#' @param base_piecewise_model Fitted model object from relsurv using piecewise constant baseline hazard.
...
#' @export
sim_predefined_obs_func_cusum_prop_threshold_piecewise_function_parallel <- function(
    sim_obs_func,
    base_piecewise_model = NULL,
    n_iterations,
    n_cores = NULL,
    rho_vec,
    pop_data_male,
  ...
  store_matrix <- foreach::foreach(
    ...
    if (is.null(base_piecewise_model)) {
      sim_obs_run <- sim_obs_func()
    } else {
      sim_obs_run <- sim_obs_func(base_piecewise_model) # relevant in case the estimated regression coefficients are used, then the model object from relsurv needs to be an input too for completeness
    }
    ...
    for (j in 1:n_elements_rho_vec) {
      cusum_chart <- cusum_prop_r_t_piecewise(
        partition_t_vec = sim_obs_run$partition_t_vec,
        baseline_vec = sim_obs_run$baseline_vec,
        start_year = sim_obs_run$start_year,
        age_vec = sim_obs_run$age_vec,
