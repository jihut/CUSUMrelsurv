...
#' Function to simulate threshold for CUSUM with (exact) additive alternative under piecewise constant baseline hazard. 
#' 
#' Function to simulate threshold for CUSUM with (exact) additive alternative under piecewise constant baseline hazard (with covariates simulating via prespecified instructions).
...
#' @export
sim_predefined_obs_func_cusum_add_exact_threshold_piecewise_function_parallel <- function(
    sim_obs_func,
    n_iterations,
    n_cores = NULL,
    gamma_vec,
  ...
  store_matrix <- foreach::foreach(
    ...
    for (j in 1:n_elements_gamma_vec) {
      cusum_chart <- cusum_add_exact_r_t_piecewise(
        partition_t_vec = sim_obs_run$partition_t_vec,
        baseline_vec = sim_obs_run$baseline_vec,
        start_year = sim_obs_run$start_year,
        age_vec = sim_obs_run$age_vec,
