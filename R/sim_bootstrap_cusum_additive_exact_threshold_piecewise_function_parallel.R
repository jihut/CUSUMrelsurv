...
# Function to simulate the max values of the CUSUM chart under in-control to estimate the threshold based on this result with the (exact) additive alternative.  
...
#' 
#' Function to simulate threshold for CUSUM with additive alternative (exact) under piecewise constant baseline hazard (with covariates simulated via bootstrapping)
...
#' @export
sim_bootstrap_cusum_add_exact_threshold_piecewise_function_parallel <- function(
    partition_t_vec,
    baseline_vec,
    n_iterations,
    n_cores = NULL,
  ...
  store_matrix <- foreach::foreach(
    ...
    for (j in 1:n_elements_gamma_vec) {
      cusum.chart <- cusum_add_exact_r_t_piecewise(
        partition_t_vec = partition_t_vec,
        baseline_vec = baseline_vec,
